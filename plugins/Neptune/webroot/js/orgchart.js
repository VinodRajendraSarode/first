...
(function ($) {
	...
	function buildNode($node, $appendTo, level, opts) {
		var $table = $("<table cellpadding='0' cellspacing='0' border='0' style='border-collapse:initial;'/>");
		var $tbody = $("<tbody/>");
		...
		if ($childNodesCount > 0) {
			...
			$nodeCell.append('<div><img class="cover" src="/neptune/img/orgchart.minus.png"/></div>');
		}
		...
		if ($childNodesCount > 0) {		
			$nodeDiv.next().children('img.cover').click(function () {
				...
				if ($tr.hasClass('contracted')) {
					...
					$(this).attr('src', '/neptune/img/orgchart.minus.png');
					...
					$(this).attr('src', '/neptune/img/orgchart.plus.png');
		...
		if ($node.attr('class') != undefined) {
			...
			$.each(classList, function (index, item) {
				if (item == 'collapsed') {
					...
					$nodeRow.find('img.cover').attr('src', '/neptune/img/orgchart.plus.png');
				} else {
