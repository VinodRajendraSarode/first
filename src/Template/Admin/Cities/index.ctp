		...
		<div class="col-md-12">
			<div class="table-responsive">
				<table class="table table-hover">
                    <thead>
                        <tr>
                            <th scope="col"><?= $this->Paginator->sort('country_id','Country')?></th>
							<th scope="col"><?= $this->Paginator->sort('state_id','Province')?></th>
                            <th scope="col"><?= $this->Paginator->sort('city')?></th>
					        <th scope="col" width="15%"><?= $this->Paginator->sort('Actions') ?></th>
				        </tr>
                    </thead>
                    <tbody>
                        <?php foreach ($cities as $city): //debug($city);exit; ?>
                        <tr>
                            <td><?= $city->country->country?></td>
							<td><?= $city->state->state?></td>
                            <td><?= $city->city?></td>
					        <td class="actions">
                                <?= $this->N->editLink(['action' => 'edit', $city->id]); ?>
                                <?= $this->N->deleteLink(['action' => 'delete', $city->id]); ?>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
