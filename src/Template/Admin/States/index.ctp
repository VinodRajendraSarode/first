				...
				<table class="table table-hover">
                    <thead>
                        <tr>
                            ...
                            <th scope="col"><?= $this->Paginator->sort('state','Province')?></th>
					        <th scope="col" width="15%"><?= $this->Paginator->sort('Actions') ?></th>
				        </tr>
                    </thead>
                    <tbody>
                        <?php foreach ($states as $state): //debug($city);exit; ?>
                        <tr>
							<td><?= $state['country']['country'] ?></td>
							<td><?= $state->state ?></td>
					        <td class="actions">
                                <?= $this->N->editLink(['action' => 'edit', $state->id]); ?>
                                <?= $this->N->deleteLink(['action' => 'delete', $state->id]); ?>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
