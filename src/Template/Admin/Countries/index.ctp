	...
	<script>
		$(document).ready(function(){
			$("#SearchForm").one("submit", function(e) {
				e.preventDefault();
				$.post('countries', $('#SearchForm').serialize()).done(function(data){
					NProgress.start();
		...
		<div class="col-md-12">
			<div class="table-responsive">
				<table class="table table-hover">
                    ...
                    <tbody>
                        <?php foreach ($countries as $country): //debug($category);exit; ?>
                        <tr>
                            <td><?= $country->country?></td>
					        <td class="actions">
                                <?= $this->N->editLink(['action' => 'edit', $country->id]); ?>
                                <?= $this->N->deleteLink(['action' => 'delete', $country->id]); ?>
                            </td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
